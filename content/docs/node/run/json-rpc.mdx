...
```bash
git clone https://github.com/shardeum/relayer-distributor.git
cd relayer-distributor
```
...
```bash
"ARCHIVER_DB_PATH": "<Base-Dir>/server/instances/archiver-db-4000/archiverdb-4000.sqlite3"
"DATA_LOG_DIR": "<Base-Dir>/server/instances/data-logs/127.0.0.1_4000"
```
...
```bash
git clone https://github.com/shardeum/relayer-collector.git collector
cd collector
```
...
You can test the setup by sending transactions on your local Shardeum network and querying them via RPC. The JSON-RPC server forwards requests to the Collector, which fetches data from the Distributor. If configured correctly, you'll get the full transaction details in the response.