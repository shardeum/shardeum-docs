...
- Byzantine Fault Tolerant consensus (Tendermint BFT) with deterministic finality,
...
The execution layer provides an EVM-compatible environment for processing transactions and executing smart contracts. It is responsible for:
...
By preserving strong EVM compatibility, this layer enables developers to deploy existing Solidity-based contracts and use familiar Ethereum tooling without protocol-specific changes.
...
This layer ensures that all nodes maintain a consistent view of the network state and supports efficient state access, synchronization, and recovery.
