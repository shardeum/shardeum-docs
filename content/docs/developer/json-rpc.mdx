...
For information on the RPC endpoint for the live network, visit [Shardeum Network Endpoints](/docs/developer/network-and-rpc) or use the RPC endpoint for our beta network or local development version.
...
2. `QUANTITY|TAG` - An integer block number, or the string `"latest"`/`"earliest"`
...
3. `QUANTITY|TAG` - An integer block number, or the string `"latest"`/`"earliest"`
...
1. `QUANTITY|TAG` - An integer block number, or the string `"latest"`/`"earliest"`
...
<Callout title="Note" type="info">
  ...
  transaction was proposed in a block, when you created a contract.
</Callout>
