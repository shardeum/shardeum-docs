...
## How Testnet Infrastructure Works

Shardeum testnet is supported by a network of validator nodes and full nodes that maintain the blockchain, validate transactions, and serve data to applications.

- **Validator nodes** participate in consensus and block production  
- **Full nodes** verify and relay blockchain data  
- **RPC nodes** provide JSON-RPC endpoints for wallets, dApps, and integrations  

For most users, the public RPC endpoint provided below is sufficient to get started.

If you need greater reliability, control, or are testing infrastructure-level integrations, you can run your own node:

- [Run a Full Node](/docs/run-a-node-testnet/run-a-full-node) 
- [Run an RPC Node](/docs/run-a-node-testnet/run-an-rpc-node)  
- [Run a Validator Node](/docs/run-a-node-testnet/run-a-validator-node)
